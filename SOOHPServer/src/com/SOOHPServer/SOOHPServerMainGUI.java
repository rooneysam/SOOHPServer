	...
	//then the button action handlers 
		...
		public static class nqSaveHandler extends MouseAdapter {
			public void mouseReleased(MouseEvent e) {
				...
				//if the three word boxes don't contain illegal characters and have text
				if(!((validateWordText(word1.getText())==true)&&(validateWordText(word2.getText())==true)&&(validateWordText(word3.getText())==true))){
					 middleLabel.setText("only letters and numbers allowed no spaces or symbols");
				}
				//if the question text doesn't contain illegal characters and has text
				else if(!(validateQuestionText(questionTextArea.getText())==true)){
					bottomLabel.setText("dont put a , or a ; in here");
				}
				//append the question to allQuestions.csv in the application folder and in the updates folder
				else{
					...
					try {
						...
						writer = new PrintWriter(new FileWriter(pathApplication+"allQuestions.csv",true));
						
						thisQuestion = new String(selectedType);
						writer.println(thisQuestion+","+word1.getText()+"."+word2.getText()+"."+word3.getText()+","+questionTextArea.getText());
						writer.close();
